...
const StatusSticker = styled(Box)(({ bgcolor }) => ({
  position: "absolute",
  top: 16,
  left: 16,
  background: bgcolor || "#757575",
  color: "white",
  padding: "6px 12px",
  borderRadius: "4px",
  fontWeight: "bold",
...
interface AdminPublicOfferCardProps {
  offer: TripResponseType
  onClick: (id: string) => void
  isTopOffer?: boolean
}

const AdminPublicOfferCard: React.FC<AdminPublicOfferCardProps> = ({ offer, onClick, isTopOffer = false }) => {
  ...
  const getStatusInfo = (status: string) => {
    switch (status?.toLowerCase()) {
      case "draft":
        return { color: statusColors.draft, label: "Juodraštis" }
      case "confirmed":
        return { color: statusColors.confirmed, label: "Patvirtintas" }
      default:
        return { color: "#757575", label: status || "Nežinoma" }
    }
  }
    ...
    <StyledCard>
      ...
      >
        <Box sx={{ position: "relative", height: 200 }}>
          ...
          <StatusSticker bgcolor={statusInfo.color}>
        ...
        <CardContent sx={{ p: 2, flexGrow: 1 }}>
          <Box sx={{ display: "flex", flexDirection: "column", height: "100%" }}>
            ...
            {offer.agentFirstName && offer.agentLastName && (
...
export default AdminPublicOfferCard
